...
- Dynamic upcoming-runs section powered by `data/runs.json`
- Run detail pages for Saturday and Wednesday meetups
- Calendar links for Google Calendar and Apple/ICS
...
Recurring runs live in `data/runs.json`. The homepage uses that file to:

- calculate the next upcoming meetup
- rotate/shuffle the recurring run cards
- update the sticky mobile next-run CTA
- generate Google Calendar and Apple/ICS calendar links

Current detail pages:

- `runs/saturday-cetate-run.html`
- `runs/wednesday-evening-run.html`
- `archive.html` is a prepared archive preview for future special runs
...
- Add special one-off runs to `data/runs.json` or create new detail/archive pages when the club starts hosting events beyond the weekly schedule
